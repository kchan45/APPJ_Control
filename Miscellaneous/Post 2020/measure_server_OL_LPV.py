...
print(instr)
print(instr.open())
# To find devices uncomment one of the lines below
#print(usbtmc.list_devices())
#lsusb
print("------TROUBLESHOOT HERE--------")
#print(float(instr.ask("MEAS:VMAX?")))
while not (instr.timeout == 0.5 and instr.rigol_quirk == False):
    instr.timeout = 0.5
    instr.rigol_quirk = False
idg = ''
print(instr.ask("*IDN?".decode('ascii')))
while not idg:
    try:
        idg = instr.ask("*IDN?")
...
V_list=[3.5, 3.0, 3.7, 4.5, 3.5, 2.0, 3.5]
V_list=[1.5, 1.5, 1.5, 3.0]
X_pos_list=[0, 10., 0, 10., 0, 10.]
O=0.
...
#        if (time.time()-t_move_now)>=t_move and (time.time()-t_move_now)<=t_move+1.3:
#            move=1
#        else:
#            move=0
  ...
  #          t_move_dis=time.time()
   ...
   #         t_move_dis=time.time()
       ...
       # print(msg)
        ...
        tSampling=1.3
        if tm_el<tSampling:
            time.sleep(tSampling-tm_el)
            tm_el=tSampling

        save_fl.write('{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f}\n'.format(time.time(),Tset,Ts,Ts2,Ts3,P,Imax,Ip2p,O777,O845,N391,He706,sum_int,*U_m,q_o,D_c,x_pos,y_pos,T_emb,V,P_emb,Prms,Rdel,Is,sig,sig2,r[0],Tspec[0]*550,t_step))
        save_fl.flush()
