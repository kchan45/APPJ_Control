...
def sendInputsArduino(arduino,appliedPower,flow,dutyCycle, arduinoAddress,pauseTime=0.05):
    ...
    # time.sleep(0.05)
    subprocess.run('echo "p,{:.2f}" > '.format(dutyCycle) + arduinoAddress, shell=True)
    # subprocess.run('echo "p,{:.2f}" > '.format(dutyCycle) + arduinoAddress, shell=False)
...
def getArduinoAddress():
    ...
    print('Getting devices that under the path: /dev/tty.usbmodem* ...')
    ...
    if len(devices)>1:
        ...
        devIdx = input('Please input the index of the device that corresponds to the master Arduino (first index = 0):\n')
    else:
...
def appjStartup4Matlab(runOpts):
    '''
    function to startup the APPJ setup for a MATLAB-interfaced experiment run

    Inputs:
    runOpts                run options for experiments

    Outputs:
    arduinoAddress        arduino address
    arduinoPI            serial connection to arduino object
    instr                oscilloscope instrument
    spec                 spectrometer device object
    dev, ctx             thermal camera device and context objects, respectively
    ioloop                 asynchronous measurement proactor event loop
    tasks                 completed tasks from an intial run of the asynchronous measurement
    '''
    ...
    flowIn = 1.5
    # send startup inputs
    sendInputsArduino(arduinoPI, powerIn, flowIn, dutyCycleIn, arduinoAddress, pauseTime=0.1)
    ...
    return [arduinoAddress, arduinoPI, instr, spec, dev, ctx, ioloop, tasks]

def appjShutdown4Matlab(dev, ctx, instr):
    '''
    function to "shutdown" the APPJ setup for a MATLAB-interfaced experiment
    run; closes thermal camera and oscilloscope connections
...
def appjSendInputs4Matlab(arduinoPI, powerIn, flowIn, dutyCycleIn, arduinoAddress):
    '''
    function to wrap the sendInputsArduino function for MATLAB interfacing

    Inputs:
    same as sendInputsArduino
...
def asyncMeas4Matlab(ioloop, arduinoPI, prevTime, instr, spec, runOpts):
    '''
    function that wraps the asynchronous measurement and extracts data from
    completed tasks

    Inputs:
    ioloop                 asynchronous measurement proactor event loop
    prevTime            previous measurement time
    arduinoPI            serial connection to arduino object
    instr                oscilloscope instrument
    spec                 spectrometer device object
    runOpts             run options object

    Outputs:
    tasks                 completed tasks from an intial run of the asynchronous measurement (unpacked)
    [Ts, Ts2, Ts3]        unpacked thermal camera measurements
    [totalIntensity, intensitySpectrum, wavelengths, meanShift]
                        unpacked spectrometer measurements
    oscOut                oscilloscope measurements
    arduinoOut            embedded measurements
    measTime            embedded measurement time
    '''
    ...
    measTime = arduinoOut[0]

    return [tasks, [Ts, Ts2, Ts3], [totalIntensity, intensitySpectrum, wavelengths, meanShift], oscOut, arduinoOut, measTime]
